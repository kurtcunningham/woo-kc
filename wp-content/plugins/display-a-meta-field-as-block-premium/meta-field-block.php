<?php
/**
 * Plugin Name: Meta Field Block Premium
 ...
 *
 * @fs_premium_only /src/premium-only/, /src/premium.js, /build/premium.js, /build/style-premium.css, /build/premium.asset.php, /build/premium-only, includes/pro
 */
...
if ( function_exists( __NAMESPACE__ . '\mfb_fs' ) ) {
	mfb_fs()->set_basename( true, __FILE__ );
	return;
}
...
// Load Pro files.
if ( mfb_fs()->is__premium_only() ) {
	require_once __DIR__ . '/includes/pro/index.php';
}
...
if ( ! class_exists( MetaFieldBlock::class ) ) :
	...
	class MetaFieldBlock {
		...
		public function register_components() {
			...
			$core_components = [ RestFields::class, ACFFields::class, DynamicField::class, Settings::class, FreemiusConfig::class ];
		...
		public function run() {
			...
			add_action( 'save_post', [ $this, 'flush_cache' ], 10, 2 );
		...
		public function render_block( $attributes, $content, $block ) {
			...
			if ( $is_dynamic_block ) {
				if ( 'acf' === $field_type ) {
					...
					if ( in_array( $object_type, [ 'post', 'term', 'user' ], true ) ) {
						...
						$content = apply_filters( '_meta_field_block_get_field_value_other_type', $content, $field_name, $object_id, $object_type, $attributes, $block );
					}
			...
			return meta_field_block_get_block_markup( $content, $attributes, $block, $object_id, $object_type, $classes, $is_dynamic_block );
		}
		...
		public function get_object_type( $field_name, $attributes, $block ) {
			...
			return apply_filters( 'meta_field_block_get_object_type', $object_type, $attributes, $block );
		}
		...
		public function enqueue_debug_information( $handle ) {
			wp_add_inline_script( $handle, 'var MFBLOG=' . wp_json_encode( [ 'environmentType' => $this->is_debug_mode() ? 'development' : wp_get_environment_type() ] ), 'before' );
		}
	}
